    ...
    <div class="flex justify-between">
        ...
        <div>
            <label class="relative block">
                ...
                <input wire:model='searchDepartment'
                    class="block w-full py-2 pr-3 bg-white border rounded-md shadow-sm placeholder:italic placeholder:text-slate-400 border-slate-300 pl-9 focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm"
        ...
        <table class="table table-xs">
            ...
            <tbody>
                @foreach ($Department as $index => $item)
                    <tr class="text-center">
                        <th>{{ $Department->firstItem() + $index }}</th>
                        <td>{{ $item->name }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">

                                <label wire:click="update_Department({{ $item->id }})"
