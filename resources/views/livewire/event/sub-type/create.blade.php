<div>
    ...
    <div class="flex gap-2">
        <label for="addEventSubType" class="btn btn-sm btn-square btn-info "><svg xmlns="http://www.w3.org/2000/svg"
        ...
        <label for="uploadEventSubType" class="btn btn-sm btn-square btn-warning tooltip-warning tooltip-right tooltip "
            data-tip="Import">
    ...
    <input type="checkbox" id="addEventSubType" class="modal-toggle" />
    <div class="modal ">
        <div class="modal-box">
            ...
            <form wire:submit.prevent='store'>
                ...
                <div class="w-full max-w-xs form-control">
                    ...
                    <select wire:model='eventType_id'
                        class=" @error('eventType_id') border-rose-500 border-2 @enderror select select-success w-full select-sm max-w-xs focus:outline-none focus:border-success focus:ring-success focus:ring-1">
                        <option selected>Select event type</option>
                        @foreach ($EventType as $item)
                            <option value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                    <x-input-error :messages="$errors->get('eventType_id')" class="mt-0" />
                </div>
                <div class="w-full max-w-xs form-control">
                ...
                <div class="modal-action">
                    ...
                    <label for="addEventSubType" class="btn btn-xs btn-error">Close!</label>
                </div>
            </form>
        </div>
    </div>

    <input type="checkbox" id="uploadEventSubType" class="modal-toggle" />
    <div class="modal">
        <div class="modal-box">
            <div class="divider divider-primary">Import Event Type</div>
            <form wire:submit.prevent='uploadEventSubTypes'>
                @csrf
                ...
                <div class="modal-action">
                    <button type="submit" class="text-white btn btn-success btn-xs">Save
                        ...
                        <svg wire:loading wire:target="uploadEventSubTypes"
                            wire:loading.class="loading loading-spinner loading-xs">
                            <!-- ... -->
                        </svg>
                    </button>
                    <label for="uploadEventSubType" class="btn btn-xs btn-error">Close!</label>
                </div>
            </form>
