<div>
    ...
    <div class="flex justify-between mx-4">
        <div>
            @livewire('event.sub-type.create')
        </div>
        <div class=" md:join">
            ...
            <input wire:model='searchEventSubType'
    ...
    <div class="mx-4 mt-4 overflow-x-auto rounded-t-lg rounded-b-lg shadow-md">
        ...
        <table class="table table-xs">
            ...
            <tbody>
                @forelse ($SubType as $index => $item)
                    <tr class="text-center">
                        <th>{{ $SubType->firstItem() + $index }}</th>
                        <td>{{ $item->EventType->name }}</td>
                        <td>{{ $item->name }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">

                                <label wire:click="update_EventSubType({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="deleteEventSubType({{ $item->id }})"
                ...
                @empty
                    <tr class="text-center">
                        <td class="text-rose-500" colspan="4">
                            <p class="flex justify-center font-semibold"> Data not found <span
    ...
    <div>{{ $SubType->links() }}</div>
    @livewire('event.sub-type.update')
