<div>
    ...
    <div class="flex justify-between mx-4">
        ...
        <div class=" md:join">
            ...
            <select wire:model='searchEventCategory'
                ...
                <option value="" selected>Search Event Category...</option>
                @foreach ($EventCategory as $item)
                    <option value="{{ $item->name }}">{{ $item->name }}</option>
                @endforeach
            </select>

            <input wire:model='searchEventType'
    ...
    <div class="mx-4 mt-4 overflow-x-auto rounded-t-lg rounded-b-lg shadow-md">
        ...
        <table class="table table-xs">
            ...
            <tbody>
                @forelse ($EventType as $index => $item)
                    <tr class="text-center">
                        <th>{{ $EventType->firstItem() + $index }}</th>
                        <td>{{ $item->EventCategory->name }}</td>
                        <td>{{ $item->name }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">

                                <label wire:click="update_EventType({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="deleteEventType({{ $item->id }})"
