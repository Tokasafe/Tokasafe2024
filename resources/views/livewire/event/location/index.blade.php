<div>
    ...
    <div class="flex justify-between">
        <div>
            @livewire('event.location.create')
        </div>
    ...
    <div class="mx-4 mt-4 overflow-x-auto rounded-md shadow-md">
        <table class="table table-xs">
            ...
            <tbody>
                @forelse ($EventLocation as $index => $item)
                    <tr class="text-center">
                        <th>{{ $EventLocation->firstItem() + $index }}</th>
                        <td class="capitalize">{{ $item->name }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">
                                <label wire:click="update_EventLocation({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="delete_EventLocation({{ $item->id }})"
    ...
    <div>{{ $EventLocation->links() }}</div>
    @livewire('event.location.update')
