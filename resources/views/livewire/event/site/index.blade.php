<div>
    ...
    <div class="mx-4 mt-4 overflow-x-auto rounded-md shadow-md">
        <table class="table table-xs">
            ...
            <tbody>
                @forelse ($EventSite as $index => $item)
                    <tr class="text-center">
                        <th>{{ $EventSite->firstItem() + $index }}</th>
                        <td>{{ $item->name }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">
                                <label wire:click="update_EventSite({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="delete_EventSite({{ $item->id }})"
    ...
    <div>{{ $EventSite->links() }}</div>
    @livewire('event.site.update')
