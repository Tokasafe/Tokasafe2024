<div>
    ...
    <div class="flex justify-between">
        <div>
            @livewire('risk.consequence.create')
        </div>
        ...
        <div>
            <label class="relative block">
                ...
                <input wire:model='search'
                    ...
                    placeholder="Search Consequence..." type="text" name="search" />
            </label>
        </div>
    </div>
    <div class="mx-4 mt-4 overflow-x-auto rounded-t-lg rounded-b-lg shadow-md">
        <table class="table table-xs ">
            ...
            <tbody>
                @foreach ($Consequence as $index => $item)
                    <tr class="text-center">
                        <th>{{ $Consequence->firstItem() + $index }}</th>
                        ...
                        <td>
                            <div class="flex flex-row justify-center gap-1">
                                ...
                                <label wire:click="update_Consequence({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="delete_Consequence({{ $item->id }})"
    ...
    <div>{{ $Consequence->links() }}</div>
    @livewire('risk.consequence.update')
