<div>
    ...
    <div class="flex justify-between">
        <div>
            @livewire('risk.likelihood.create')
        </div>
    ...
    <div class="mx-4 mt-4 overflow-x-auto rounded-t-lg rounded-b-lg shadow-md">
        <table class="table table-xs ">
            ...
            <tbody>
                @foreach ($Likelihood as $index => $item)
                    <tr class="text-center">
                        <th>{{ $Likelihood->firstItem() + $index }}</th>
                        ...
                        <td>
                            <div class="flex flex-row justify-center gap-1">
                                ...
                                <label wire:click="update_Likelihood({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="delete_Likelihood({{ $item->id }})"
    ...
    <div>{{ $Likelihood->links() }}</div>
    @livewire('risk.likelihood.update')
